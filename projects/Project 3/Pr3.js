...
const isTouchDevice = () => {
    try {
        // We try to create TouchEvent (it fails for desktops and throws an error)
...
canvas.addEventListener(events[deviceType].enter, function (e) {
    e.preventDefault();
    hovering = true;
});
// Function on mouseleave
canvas.addEventListener(events[deviceType].leave, function (e) {
    e.preventDefault();
    hovering = false;
});
...
function Particle() {
    this.width = randomNumberGenerator(0.1, 0.5) * 5;
    this.height = randomNumberGenerator(0.1, 0.5) * 5;
